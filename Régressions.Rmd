---
title: "Résumé sur les régressions, M1 et M2 QESS"
...
# Régressions linéaires (cours de Jérôme Deauvieau)

$$Y_i = \beta_0 + \beta_1X_i + \epsilon_i$$ pour $i = 1,...,n$

La **régression linéaire** prend comme variable dépendante/à expliquer ($var\_dep$) une variable quantitative continue, telle que la température, le PIB, mon envie de dormir progressive pendant les cours de Dhuot...les variables explicatives/indépendantes peuvent être quanti ou quali.
...
On veut voir si l'ajout de ces interactions est statistiquement significative (c'est la méthode de Deauvieau, Parodi a la sienne [que je détaille plus bas](#analyse-de-deviance)). On effectue un test statistique sur l'interaction, qui est distribué comme une loi du khi-deux, sur la différence de vraisemblance entre les modèles d'indépendance et d'interaction.
...
# Régression log-linéaire (cours de Maxime Parodi)
...
# Approches expérimentales (cours d'Olivier Godechot)
...
- Limite suppose que la forme fonctionnelle est la même de part et d'autre du seuil.
...
# Analyses longitudinales (cours de Loup Wolff)

Les données de panel varient dans deux dimensions : la dimension interindividuelle et la dimension intraindividuelle ou temporelle.  
$$y_{it} = x_{it}\beta + \alpha_i + \epsilon_{it} \\ \qquad i=1,...,N \qquad t = 1,...,T $$
où chaque variable est indexée à la fois par individu et par période. Modélisation avec effet fixe individuel au modèle linéaire classique, censé capter l'effet des caractéristiques individuelles constantes dans le temps sur la variable d'intérêt, avec :  
- $y_{it}$ la variable d'intérêt,  
- $x_{it}$ un vecteur (ligne) de $K$ variables explicatives observées sur l'individu $i$ à la date $t$,  
- $\beta$ l'effet de ces variables (soit un vecteur de paramètres de dimension $K$),  
- $\alpha_i$ l'effet fixe individuel.
